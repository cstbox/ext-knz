...
from pycstbox.hal.drivers import knz
from pycstbox.knz import smp
...
class TestSMPHalDriver(_BaseTestCase):
    def setUp(self):
        ...
        cfg_dev.outputs = {
            'Irr' : {
                'varname' : 'irradiance_test',
                'enabled' : True
            },
            'temp' : {
                'varname' : 'temperature_test',
                'enabled' : True
            }
        }

        self.set_hal_device(knz.SMP(coord=coord, cfg=cfg_dev))
...
class TestSCSHalDriver(_BaseTestCase):
    def setUp(self):
        ...
        self.set_hal_device(solea.SCS(cfg_coord=cfg_coord, cfg_dev=cfg_dev))
